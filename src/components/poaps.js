...
export const DisplayPOAPs = (props) => {
  ...
  const getPoaps = async (address) => {
    ...
    const poapDisplay = poaps.map(poap => (
      <div key={poap['event']['id']} style={{height:'126px'}}>
        <img src={poap['event']['image_url']} alt={`POAP for ${poap['event']['name']}`} style={{margin:'10px', height:'100px', minWidth:'100px'}} />
      </div>
    ))
    return poapDisplay;
  }
  ...
  return (
    <div style={{display:'flex', flexDirection:'row', justifyContent: 'center', overflowX: 'scroll'}}>
    {poaps}
    </div>
  )
}